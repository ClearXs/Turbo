...
public class BoDomainController {
    ...
    public R<Boolean> save(@PathVariable("boId") Long boId, @RequestBody GeneralDomainObject domain) throws BizException {
    ...
    public R<Boolean> edit(@PathVariable("boId") Long boId, @RequestBody GeneralDomainObject domain) throws BizException {
    ...
    public R<Boolean> saveOrUpdate(@PathVariable("boId") Long boId, @RequestBody GeneralDomainObject domain) throws BizException {
    ...
    public R<Boolean> batchSaveOrUpdate(@PathVariable("boId") Long boId, @RequestBody List<GeneralDomainObject> domain) throws BizException {
    ...
    public R<GeneralDomainObject> details(@PathVariable("boId") Long boId, @PathVariable("id") Long id) throws BizException {
        GeneralDomainObject entity = domainService.getById(boId, id);
        return R.ok(entity);
    }

    @Operation(summary = "列表")
    @PostMapping("/list/{boId}")
    public R<List<GeneralDomainObject>> list(@PathVariable("boId") Long boId, @RequestBody QueryParam<GeneralDomainObject> params) throws BizException {
        QueryWrapper<GeneralDomainObject> wrapper = Wrappers.query();
        domainService.getBoRepositoryOrThrow(boId)
                .aspectOn("list", context ->
                        Optionals.withBoth(context.getTypeFirst(QueryWrapper.class), context.getTypeFirst(BoSchema.class))
                                .ifPresent(pair -> {
                                    QueryWrapper<GeneralDomainObject> queryWrapper = pair.getFirst();
                                    BoSchema boSchema = pair.getSecond();
                                    DomainConditions.domainQuery(boSchema, queryWrapper, params);
                                })
                );
        List<GeneralDomainObject> entities = domainService.list(boId, wrapper);
        return R.ok(entities);
    }

    @Operation(summary = "分页")
    @PostMapping("/page/{boId}")
    public R<IPage<GeneralDomainObject>> page(@PathVariable("boId") Long boId, @RequestBody QueryParam<GeneralDomainObject> params) throws BizException {
        QueryWrapper<GeneralDomainObject> wrapper = Wrappers.query();
        domainService.getBoRepositoryOrThrow(boId)
        ...
        IPage<GeneralDomainObject> page = domainService.page(boId, params.getPage(), wrapper);
        return R.ok(page);
    }

    @Operation(summary = "导出")
    @PostMapping("/export/{boId}")
    public void export(@PathVariable("boId") Long boId, @RequestBody QueryParam<GeneralDomainObject> params, HttpServletResponse response) {
    ...
    public <Z extends TreeDomain<GeneralDomainObject, Z>> R<List<Z>> tree(@PathVariable("boId") Long boId, @RequestBody QueryParam<GeneralDomainObject> params) throws BizException {
        return null;
    }
    ...
    static class DomainConditions extends Conditions {
        ...
        public static <T extends GeneralDomainObject> QueryWrapper<T> domainQuery(BoSchema boSchema, QueryParam<T> queryParam) {
            return domainQuery(boSchema, Wrappers.query(), queryParam);
        }

        public static <T extends GeneralDomainObject> QueryWrapper<T> domainQuery(BoSchema boSchema, QueryWrapper<T> queryWrapper, QueryParam<T> queryParam) {
            ...
            onQueryTerm(
                    ...
                    term -> {
                        ...
                        Optional.ofNullable(fieldType.getDslType())
                                ...
                                .map(javaType -> new EntityTerm(term, GeneralDomainObject.class, field.getField(), javaType.getJavaType()))
